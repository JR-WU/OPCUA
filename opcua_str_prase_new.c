...
#define YES 1
#define S_PORT 5888 //∑¢ÀÕ∂Àø⁄
#define R_PORT 5222 //Ω” ’∂Àø⁄
#define BACKLOG 7
int s_fd = 0;
int ret = 0;
int r_fd = 0;
...
int getDataFromStr( char* const str, DATA_SOURCE *source) 
{
	char *p=NULL;
	int i;
	p = strstr(str,source->name);//strstr∫Ø ˝ «”√¿¥≈–∂œstr2 «∑Ò «str1µƒ◊”¥Æ.º¥≈–∂œsource÷–µƒname «≤ª «strµƒ◊”¥Æ°£
	if(p != NULL) {
		p = strstr(p,":");
			if(p != NULL) {
				p += 1;  //skip ":"
				if(source->type == 1) {
					...
					}//Ã·»°◊÷∑˚¥Æ ˝æ›∑≈»Îstring_data ˝◊È					
				} else if(source->type == 2) {
					source->data = atof(p);//Ω´◊÷∑˚¥Æ◊™ªª≥…∏°µ„ ˝
				} else {
					return -1;
				}
				return 1;
...
void creat_server_sockfd4(int *sockfd, struct sockaddr_in *local, int portnum){
	...
	*sockfd = socket(AF_INET, SOCK_STREAM, 0);//¥¥Ω®Ã◊Ω”ø⁄£¨Socket=Ip address+ TCP/UDP + port
	if(*sockfd < 0){
		perror("socket");
		exit(EXIT_FAILURE);
	}
	...
	if(err){
		perror("setsockopt");
	}//sockfdŒ™¥¥Ω®Ã◊Ω”◊÷µƒ∑µªÿ÷µ°£


	memset(local, 0, sizeof(struct sockaddr_in));
	local->sin_family = AF_INET;
	local->sin_addr.s_addr = htonl(INADDR_ANY);//INADDR_ANYŒ™≤ª»∑∂®µÿ÷∑£¨¥Û≤ø∑÷œµÕ≥æ˘Œ™0.0.0.0£¨ø… π”√»Œ“‚Ω”ø⁄
	local->sin_port = htons(portnum);//÷ÿ–¬…Ë÷√localµƒ÷µ,≤¢◊™ªª∏Ò Ω

	err = bind(*sockfd, (struct sockaddr*)local, sizeof(struct sockaddr_in));//local”ÎÃ◊Ω”ø⁄¡¨Ω”
	if(err < 0){
		perror("bind");
		exit(EXIT_FAILURE);
	}
	err = listen(*sockfd, BACKLOG);//»√Ã◊Ω”◊÷¥¶”⁄º‡Ã˝◊¥Ã¨
	if(err < 0){
		perror("listen");
		exit(EXIT_FAILURE);
	}
...
void creatserver(struct argument *p){
	...
	creat_server_sockfd4(&serverfd,&local_addr_s,p->port);//Ã◊Ω”◊÷”Î∑˛ŒÒ∆˜œ‡¡¨≤¢¥¶”⁄º‡Ã˝◊¥Ã¨
...
int ReadData(int fd,char *p){
	...
	ret = recv(fd, &c, 1,0);//Ω” ’Ã◊Ω”◊÷ ˝æ›≤¢copyµΩc÷∏µƒbuff÷–°£∑µªÿcopy◊÷Ω⁄ ˝°£
	if(ret < 0){
		perror("recv");
		return 0;
	}
...
{ 
	...
	int len=sizeof(info); 
	getsockopt(sock, IPPROTO_TCP, TCP_INFO, &info, (socklen_t *)&len); //ªÒ»°Ã◊Ω”ø⁄–≈œ¢
...
void* soureDataPrase(void *arg)//∂¡»° ˝æ›£¨Ω®¡¢¡¨Ω”£¨∑¢ÀÕ ˝æ›
{
	fd_set readfd;
	int ret = 0;
	int ret_send = 0;
	...
	while(1){
		maxfd = 0;//maxfd «–Ë“™º‡ ”µƒ◊Ó¥ÛµƒŒƒº˛√Ë ˆ∑˚÷µ+1
		struct timeval timeout;// struct timevalΩ·ππ”√”⁄√Ë ˆ“ª∂Œ ±º‰≥§∂»£¨»Áπ˚‘⁄’‚∏ˆ ±º‰ƒ⁄£¨–Ë“™º‡ ”µƒ√Ë ˆ∑˚√ª”– ¬º˛∑¢…˙‘Ú∫Ø ˝∑µªÿ£¨∑µªÿ÷µŒ™0°£
		timeout.tv_sec = 2;
		timeout.tv_usec = 0;
		if(recvDataFD.fd != 0){
			FD_ZERO(&readfd);//Ω´÷∏∂®µƒŒƒº˛√Ë ˆ∑˚ºØ«Âø’£¨‘⁄∂‘Œƒº˛√Ë ˆ∑˚ºØ∫œΩ¯––…Ë÷√«∞£¨±ÿ–Î∂‘∆‰Ω¯––≥ı ºªØ
			FD_SET(recvDataFD.fd, &readfd);//”√”⁄‘⁄Œƒº˛√Ë ˆ∑˚ºØ∫œ÷–‘ˆº”“ª∏ˆ–¬µƒŒƒº˛√Ë ˆ∑˚°£
			maxfd = recvDataFD.fd + 1;			
			ret = select(maxfd, &readfd, NULL, NULL, &timeout);//”√¿¥ºÏ≤È&readfdŒƒº˛√Ë ˆ∑˚ «∑Òø…∂¡°£∑µªÿœ‡”¶Œª «∑ÒŒ™“ª°£
			if(ret == -1){
				perror("select");
			}else if(ret > 0 && FD_ISSET(recvDataFD.fd, &readfd)){ //»Ùø…∂¡£¨«“recvDataFD,fd‘⁄&readfd÷–
				//printf("ret = %d\n",ret);
				p = (char *)malloc(sizeof(char) * 1000);//∏¯p÷∏’ÎÀ˘÷∏µƒµÿ÷∑∑÷≈‰4000◊÷Ω⁄µƒƒ⁄¥Êø’º‰
				...
				if(num > 0) {
					...
					//if(sendOriginalDataFD.fd > 0) {
						ret_send = send(sendOriginalDataFD.fd, p, num, 0);//send()”√”⁄œÚ“ª∏ˆ“—æ≠¡¨Ω”µƒsocket∑¢ÀÕ ˝æ›					
						if(ret_send < 0) {
							perror("send");
...
void* nodeidFindData(const UA_NodeId nodeId) 
{
	int i;
	for(i=0;i<sizeof(source)/sizeof(DATA_SOURCE);i++) {
		if(strncmp((char*)nodeId.identifier.string.data, source[i].name, strlen(source[i].name)) == 0) {//»Áπ˚Ω⁄µ„IDµƒ»œ÷§–≈œ¢”Î∑˛ŒÒ∆˜√˚◊÷œ‡Õ¨Â
			if(source[i].type == 1) {
