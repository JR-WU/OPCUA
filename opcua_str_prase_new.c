...
#define YES 1
#define S_PORT 5888 //发送端口
#define R_PORT 5222 //接收端口
#define BACKLOG 7
int s_fd = 0;
int ret = 0;
int r_fd = 0;
...
int getDataFromStr( char* const str, DATA_SOURCE *source) 
{
	char *p=NULL;
	int i;
	p = strstr(str,source->name);//strstr函数是用来判断str2是否是str1的子串.即判断source中的name是不是str的子串。
	if(p != NULL) {
		p = strstr(p,":");
			if(p != NULL) {
				p += 1;  //skip ":"
				if(source->type == 1) {
					...
					}//提取字符串数据放入string_data数组					
				} else if(source->type == 2) {
					source->data = atof(p);//将字符串转换成浮点数
				} else {
					return -1;
				}
				return 1;
...
void creat_server_sockfd6(int *sockfd, struct sockaddr_in6 *local, int portnum){
	...
	*sockfd = socket(AF_INET6, SOCK_STREAM, 0);//创建套接口，Socket=Ip address+ TCP/UDP + port
	if(*sockfd < 0){
		perror("socket");
		exit(EXIT_FAILURE);
	}
	...
	err = listen(*sockfd, BACKLOG);//让套接字处于监听状态
	if(err < 0){
		perror("listen");
		exit(EXIT_FAILURE);
	}
...
int ReadData(int fd,char *p){
	...
	ret = recv(fd, &c, 1,0);//接收套接字数据并copy到c指的buff中。返回copy字节数。
	if(ret < 0){
		perror("recv");
		return 0;
	}
...
{ 
	...
	int len=sizeof(info); 
	getsockopt(sock, IPPROTO_TCP, TCP_INFO, &info, (socklen_t *)&len); //获取套接口信息
...
void* soureDataPrase(void *arg)//读取数据，建立连接，发送数据
{
	fd_set readfd;
	int ret = 0;
	int ret_send = 0;
	...
	while(1){
		maxfd = 0;//maxfd是需要监视的最大的文件描述符值+1
		struct timeval timeout;// struct timeval结构用于描述一段时间长度，如果在这个时间内，需要监视的描述符没有事件发生则函数返回，返回值为0。
		timeout.tv_sec = 2;
		timeout.tv_usec = 0;
		if(recvDataFD.fd != 0){
			FD_ZERO(&readfd);//将指定的文件描述符集清空，在对文件描述符集合进行设置前，必须对其进行初始化
			FD_SET(recvDataFD.fd, &readfd);//用于在文件描述符集合中增加一个新的文件描述符。
			maxfd = recvDataFD.fd + 1;			
			ret = select(maxfd, &readfd, NULL, NULL, &timeout);//用来检查&readfd文件描述符是否可读。返回相应位是否为一。
			if(ret == -1){
				perror("select");
			}else if(ret > 0 && FD_ISSET(recvDataFD.fd, &readfd)){ //若可读，且recvDataFD,fd在&readfd中
				//printf("ret = %d\n",ret);
				p = (char *)malloc(sizeof(char) * 1000);//给p指针所指的地址分配4000字节的内存空间
				...
				if(num > 0) {
					...
					//if(sendOriginalDataFD.fd > 0) {
						ret_send = send(sendOriginalDataFD.fd, p, num, 0);//send()用于向一个已经连接的socket发送数据					
						if(ret_send < 0) {
							perror("send");
...
void* nodeidFindData(const UA_NodeId nodeId) 
{
	int i;
	for(i=0;i<sizeof(source)/sizeof(DATA_SOURCE);i++) {
		if(strncmp((char*)nodeId.identifier.string.data, source[i].name, strlen(source[i].name)) == 0) {//如果节点ID的认证信息与服务器名字相同�
			if(source[i].type == 1) {
